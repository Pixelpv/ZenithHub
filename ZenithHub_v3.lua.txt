--[[
╔══════════════════════════════════════════════════╗
║        ZENITH HUB  v3.0  (SINGLE FILE)          ║
║       Blox Fruits — Executor Ready               ║
║  Stealth rewrite: sem teleporte instantâneo      ║
...
-- ════════════════════════════════════════════════
-- SAVE / LOAD (Zyn Hub pattern)
-- ════════════════════════════════════════════════
local FolderName = "ZenithHub"
...
local function LoadSettings()
    ...
    if ok and res then _G.SaveData = res end
end
...
-- ════════════════════════════════════════════════
-- HOOK FUNCTIONS — padrão dos open sources
-- ════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════
-- NAMECALL HOOK (Vua Hub stealth pattern)
-- Intercepta FireServer para spoofar MousePos em skills
-- ════════════════════════════════════════════════
MousePos = Vector3.zero -- atualizado pelo loop de farm
pcall(function()
    ...
    mt.__namecall = newcclosure(function(...)
        ...
        if tostring(method) == "FireServer" then
            if tostring(args[1]) == "RemoteEvent" then
                local a2 = tostring(args[2])
                if a2 ~= "true" and a2 ~= "false" then
                    if _G.AimSkill then
                        args[2] = MousePos
                        return old_nc(table.unpack(args))
                    end
                end
            end
        end
...
pcall(function()
    ...
    if wo then local f = wo:FindFirstChild("Foam;") if f then f:Destroy() end end
    local rk = workspace:FindFirstChild("Rocks") if rk then rk:Destroy() end
end)
...
-- Controle de skills da fruta (Zyn Hub)
...
-- ════════════════════════════════════════════════
-- TELEPORTE STEALTH — SEM SALTO INSTANTÂNEO
-- ════════════════════════════════════════════════
--
--  O problema que causava kick:
--  `r.CFrame = cf` de uma vez = velocidade infinita detectada pelo servidor.
--
--  Solução: movimento em PASSOS pequenos com task.wait() entre cada passo.
--  Cada passo move no máximo TP_STEP_SIZE studs.
--  Entre passos há um delay de TP_STEP_DELAY segundos.
--  A velocidade resultante (~75 studs / 0.07s = ~1071 st/s) fica bem acima
--  do walk mas abaixo do threshold de kick do BF.
--
--  Para usar TweenService (visual mais suave, menos passos no log de network):
--  configure S.UseTween = true e S.TweenSpeed (studs/segundo).
-- ════════════════════════════════════════════════
local TP_STEP_SIZE  = 75   -- máximo de studs por passo
local TP_STEP_DELAY = 0.07 -- delay entre passos (segundos)
local TP_TWEEN_SPD  = 180  -- studs/s no modo TweenService

local UseTween = false -- pode ser alterado pela UI
...
local function tweenTo(cf, spd)
    ...
    local t = math.clamp(dist / (spd or TP_TWEEN_SPD), 0.05, 3)
    local tw = TweenService:Create(r, TweenInfo.new(t, Enum.EasingStyle.Linear), {CFrame = cf})
    tw:Play()
    tw.Completed:Wait()
end

local function steppedTp(cf)
    local r = Root()
    if not r then return end
    local from = r.CFrame
    local dist = (from.Position - cf.Position).Magnitude
    if dist <= TP_STEP_SIZE then
        r.CFrame = cf
        return
    end
    local steps = math.ceil(dist / TP_STEP_SIZE)
    for i = 1, steps do
        local r2 = Root()
        if not r2 then break end
        local t = i / steps
        r2.CFrame = from:Lerp(cf, t)
        if i < steps then task.wait(TP_STEP_DELAY) end
    end
end

-- Ponto de entrada público — respeita a configuração UseTween
_tp = function(cf)
    if UseTween then
        tweenTo(cf)
    else
...
notween = steppedTp  -- alias de compatibilidade (sem tween mesmo com UseTween=true)
...
-- ════════════════════════════════════════════════
-- BRING ENEMY — com cooldown e range limit
-- ════════════════════════════════════════════════
local _bringCooldown = 0

BringEnemy = function()
    if not _B then return end
    local now = tick()
    if now - _bringCooldown < 0.08 then return end
    _bringCooldown = now

    for _, mob in pairs(workspace.Enemies:GetChildren()) do
        if mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob.PrimaryPart then
            if (mob.PrimaryPart.Position - PosMon).Magnitude <= _G.BringRange then
...
statsSetings = function(stat, amount)
    if plr.Data.Points.Value ~= 0 then
        ...
        pcall(function() CommF_:InvokeServer("AddPoint", map[stat] or stat, amount or 1) end)
    end
end

-- ════════════════════════════════════════════════
-- KILL FUNCTIONS (tabela f)
-- RandomCFrame usa delays maiores (0.3-0.5s) para evitar detecção
-- ════════════════════════════════════════════════
local function lockAndBring(mob)
...
local function randomJitter(hrp)
    -- jitter aleatório em volta do mob para parecer movimento orgânico
    -- delay aumentado (0.3s) = ~53 studs/s, bem abaixo do threshold
    local function rv() return math.random(-25, 25) end
    task.wait(0.3)  _tp(hrp.CFrame * CFrame.new(rv(), 30, rv()))
    task.wait(0.3)  _tp(hrp.CFrame * CFrame.new(rv(), 30, rv()))
    task.wait(0.35) _tp(hrp.CFrame * CFrame.new(rv(), 30, rv()))
    task.wait(0.3)  _tp(hrp.CFrame * CFrame.new(rv(), 30, rv()))
    task.wait(0.35) _tp(hrp.CFrame * CFrame.new(rv(), 30, rv()))
end
...
f.Kill = function(mob, toggle)
    ...
    local hrp = lockAndBring(mob) if not hrp then return end
    ...
    if RandomCFrame then randomJitter(hrp) end
end
...
f.KillSea = function(mob, toggle)
    ...
    if tool.ToolTip == "Blox Fruit" then
        ...
        -- Sea Beast: sobe gradualmente (stepped já cuida disso)
...
-- ════════════════════════════════════════════════
-- SISTEMA DE PRIORIDADE (exclusive vs passive)
-- ════════════════════════════════════════════════
local ExThread = nil
local ExName   = nil
local ExPrio   = 999
local PassiveThreads = {}

local function stopExclusive()
    if ExThread then pcall(task.cancel, ExThread) end
    ExThread = nil ExName = nil ExPrio = 999 _B = false
end

local function startExclusive(name, prio, fn)
    if prio <= ExPrio then
        stopExclusive()
        ExName = name ExPrio = prio
    ...
    startExclusive("AutoRaid", 2, function()
        while ExName == "AutoRaid" do
            if alive() then
                for _, mob in ipairs(workspace.Enemies:GetChildren()) do
                    local hm = mob:FindFirstChildOfClass("Humanoid")
                    if hm and hm.Health > 0 then f.Kill(mob, true) break end
                end
            end
            task.wait(0.15)
        end
...
local function setAutoFarm(on)
    ...
    startExclusive("AutoFarm", 4, function()
        while ExName == "AutoFarm" do
            ...
            -- TP até o NPC da quest em passos (não instantâneo)
            _tp(q.cfNPC)
            task.wait(0.4)
            ...
            -- Atualiza MousePos para o hook namecall
            local hrp = mob and mob:FindFirstChild("HumanoidRootPart")
            if hrp then MousePos = hrp.Position end
            task.wait(0.1)
        end
    end)
end
            ...
            for _, v in ipairs(workspace:GetChildren()) do
                if v:IsA("Model") and v.Name:lower():find("terrorshark") then
                    if not v:FindFirstChild("ZH_ESP") then
                        ...
                        hl.FillTransparency=0.5 hl.Parent=v
                    end
                end
            end
            task.wait(2)
        ...
        startPassive("Fly", function()
            ...
            while PassiveThreads["Fly"] do
                ...
                if r2 and FlyVel and FlyVel.Parent then
                    ...
                    FlyVel.Velocity=mv
                    FlyGyro.CFrame=CFrame.new(r2.Position, r2.Position+cf.LookVector)
                end
                task.wait(0.03)
            end
        end)
...
local function showSplash()
    pcall(function()
        ...
        local stroke=Instance.new("UIStroke",card) stroke.Color=Color3.fromRGB(80,120,255)
        stroke.Thickness=1.5 stroke.Transparency=1
        ...
        local t2=mkL(72,"Blox Fruits  •  v3.0  •  World "..WORLD,Enum.Font.Gotham,13,Color3.fromRGB(150,150,200))
        local t3=mkL(95,"Stealth Mode Ativo",Enum.Font.Gotham,11,Color3.fromRGB(80,200,120))
        local barBg=Instance.new("Frame",card)
        barBg.AnchorPoint=Vector2.new(0.5,0) barBg.Position=UDim2.fromOffset(210,130)
        ...
        task.wait(0.3)
        for _,item in ipairs({t1,t2,t3}) do tw(item,0.4,{TextTransparency=0}) end
...
local function buildUI()
    local W = Fluent:CreateWindow({
        Title       = "Zenith Hub",
        SubTitle    = "Blox Fruits  •  v3.0  •  World "..WORLD,
        TabWidth    = 148,
    ...
    T.Mastery   = W:AddTab({Title="Mastery",      Icon="book-open" })
    ...
    pcall(function()
        ...
        T.Farm:AddSection("Arma & Stealth")
        T.Farm:AddDropdown("WeaponSel", {Title="Arma Selecionada", Values={"(Nenhuma)", table.unpack(SWORDS)}, Default="(Nenhuma)", Callback=function(v)
            _G.SelectWeapon = v ~= "(Nenhuma)" and v or nil
        end})
        ...
        T.Farm:AddSection("Anti-Detecção (Stealth)")
        T.Farm:AddToggle("RandomCF", {Title="Random CFrame (jitter)", Default=false, Callback=function(v)
            RandomCFrame = v
        end})
        T.Farm:AddToggle("UseTw", {Title="Usar TweenService (movimento suave)", Default=false, Callback=function(v)
            UseTween = v
        end})
        T.Farm:AddSlider("StepSize", {Title="Tamanho do Passo (studs)", Min=30, Max=200, Default=75, Rounding=0, Callback=function(v)
            TP_STEP_SIZE = v
        end})
        T.Farm:AddSlider("StepDelay", {Title="Delay entre Passos (ms)", Min=30, Max=200, Default=70, Rounding=0, Callback=function(v)
            TP_STEP_DELAY = v / 1000
        end})
        T.Farm:AddSlider("MobH", {Title="Mob Height", Min=5, Max=60, Default=20, Rounding=0, Callback=function(v)
            _G.MobHeight = v
        end})
        T.Farm:AddToggle("AimSkillT", {Title="Aim Skill (spoof MousePos)", Default=false, Callback=function(v)
            _G.AimSkill = v
        end})
    end)
    ...
    pcall(function()
        ...
        T.Bosses:AddButton({Title="Teleportar ao Boss", Callback=function()
            local boss = findBoss(S.SelectedBoss)
            if boss then
                local hrp = boss:FindFirstChild("HumanoidRootPart")
                if hrp then _tp(hrp.CFrame * CFrame.new(0,20,0)) end
            else
                N("Boss","Não encontrado no mapa.",3,"Warning")
            end
        end})
    end)
    ...
    -- ══ TAB: SEA EVENTS ════════════════════════
    pcall(function()
        T.Sea:AddSection("Sea Beast / Terrorshark")
        T.Sea:AddToggle("AutoSea", {Title="Auto Sea Beast", Default=false, Callback=function(v)
        ...
        T.Sea:AddSection("Auto Raid")
        T.Sea:AddToggle("AutoRaid", {Title="Auto Raid (mobs)", Default=false, Callback=function(v)
            setAutoRaid(v)
        end})
    end)
    ...
    -- ══ TAB: MASTERY ═══════════════════════════
    pcall(function()
        T.Mastery:AddSection("Fruit Mastery")
        T.Mastery:AddToggle("MasFruit_Z", {Title="Skill Z", Default=false, Callback=function(v) _G.FruitSkills.Z=v end})
        T.Mastery:AddToggle("MasFruit_X", {Title="Skill X", Default=false, Callback=function(v) _G.FruitSkills.X=v end})
        T.Mastery:AddToggle("MasFruit_C", {Title="Skill C", Default=false, Callback=function(v) _G.FruitSkills.C=v end})
        T.Mastery:AddToggle("MasFruit_V", {Title="Skill V", Default=false, Callback=function(v) _G.FruitSkills.V=v end})
        T.Mastery:AddToggle("MasFruit_F", {Title="Skill F (Transform)", Default=false, Callback=function(v) _G.FruitSkills.F=v end})
        T.Mastery:AddSlider("HealthMas", {Title="Mudar p/ Fruta quando HP ≤", Min=100, Max=5000, Default=500, Rounding=0, Callback=function(v)
            HealthM = v
        end})
        T.Mastery:AddSection("Modo")
        T.Mastery:AddDropdown("MasType", {Title="Tipo de Mastery", Values={"Fruit","Gun"}, Default="Fruit", Callback=function(v)
            S.MasteryType = v
        end})
        T.Mastery:AddToggle("AutoMas", {Title="Auto Mastery Farm", Default=false, Callback=function(v)
            setMasteryFarm(v, S.MasteryType)
            N("Mastery", v and "Ativado!" or "Off.", 2)
        end})
    end)

    -- ══ TAB: PLAYER ════════════════════════════
    pcall(function()
        T.Player:AddSection("Movimentação")
        T.Player:AddToggle("Fly",     {Title="Fly",          Default=false, Callback=function(v) setFly(v) end})
        ...
        T.Player:AddSection("Utilidades")
        T.Player:AddToggle("AntiAFK", {Title="Anti AFK", Default=true, Callback=function(v)
            setAntiAFK(v)
        end})
        T.Player:AddButton({Title="Respawn Personagem", Callback=function()
            plr:LoadCharacter()
        end})
    end)
    ...
    -- ══ TAB: TELEPORTS ═════════════════════════
    pcall(function()
        T.Teleports:AddSection("Ilhas — World "..WORLD)
        local isleNames = {}
        for _, isle in ipairs(ISLANDS) do isleNames[#isleNames+1] = isle.name end
        T.Teleports:AddDropdown("IsleSel", {Title="Ilha Destino", Values=isleNames, Default=isleNames[1], Callback=function(v)
            S.SelectedIsland = v
        end})
        T.Teleports:AddButton({Title="Teleportar", Callback=function()
            for _, isle in ipairs(ISLANDS) do
                if isle.name == S.SelectedIsland then
                    _tp(isle.cf)
                    N("Teleport","→ "..isle.name, 2)
                    break
                end
            end
        end})
        T.Teleports:AddSection("Locais Rápidos")
        T.Teleports:AddButton({Title="Void (Fantasma Ghost)", Callback=function()
            _tp(w.Ghost)
        end})
    end)
    ...
    -- ══ TAB: VISUALS ═══════════════════════════
    pcall(function()
        T.Visuals:AddSection("Iluminação")
        T.Visuals:AddToggle("FullBright", {Title="Full Bright", Default=true, Callback=function(v)
            if v then
                Lighting.Brightness=2 Lighting.Ambient=Color3.new(0.695,0.695,0.695)
            else
                Lighting.Brightness=1 Lighting.Ambient=Color3.new(0,0,0)
            end
        end})
        T.Visuals:AddSection("Câmera")
        T.Visuals:AddSlider("FOV", {Title="Field of View", Min=70, Max=120, Default=70, Rounding=0, Callback=function(v)
    ...
    -- ══ TAB: SETTINGS ══════════════════════════
    pcall(function()
        T.Settings:AddSection("Save / Load")
